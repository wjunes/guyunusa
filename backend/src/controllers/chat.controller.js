...
export async function sendMessage(req, res) {
  ...
  if (user.plan === 'free') {
    ...
    if (count.n >= FREE_DAILY_LIMIT) {
      return res.status(HTTP_STATUS.BAD_REQUEST).json({ ok: false, message: ERRORS.RATE_LIMITED });
    }
  }
  ...
  if (!convId) {
    const newConv = db.prepare('INSERT INTO conversations (user_id, title) VALUES (?, ?)').run(userId, content.slice(0, 60));
    convId = newConv.lastInsertRowid;
  }
  ...
  try {
    ...
    db.prepare('INSERT INTO messages (conversation_id, role, content, provider, tokens_used) VALUES (?, ?, ?, ?, ?)')
      .run(convId, 'assistant', reply, provider, tokens);

    // Actualizar título si es la primera respuesta
    const msgCount = db.prepare('SELECT COUNT(*) as n FROM messages WHERE conversation_id = ?').get(convId);
    if (msgCount.n <= 2) {
      db.prepare('UPDATE conversations SET title = ? WHERE id = ?').run(content.slice(0, 60), convId);
    }

    res.json({ ok: true, conversation_id: convId, message: { role: 'assistant', content: reply }, provider });
  } catch (err) {
    res.status(HTTP_STATUS.SERVER_ERROR).json({ ok: false, message: ERRORS.AI_UNAVAILABLE });
  }
}
