...
export function renderSidebar(store) {
  ...
  el.innerHTML = `
    <div class="c-sidebar">
      ...
      <div class="c-sidebar__label">Conversaciones</div>
      <div class="c-sidebar__list" id="conv-list">
        ${convs.length === 0
          ? `<div class="c-sidebar__empty">
               Todavía no tenés conversaciones.<br>¡Empezá una!
             </div>`
          : convs.map(c => `
            <div class="c-sidebar__item ${c.id === active ? 'c-sidebar__item--active' : ''}"
              ...
              <div class="c-sidebar__item-actions">
                <button class="c-sidebar__item-action c-sidebar__item-share"
                        data-share-id="${c.id}"
                        title="Compartir conversación" aria-label="Compartir">
                  ${iconShare()}
                </button>
                <button class="c-sidebar__item-action c-sidebar__item-del"
                        data-del-id="${c.id}"
                        title="Eliminar conversación" aria-label="Eliminar">
                  ${iconTrash()}
                </button>
              </div>
            </div>`).join('')
      ...
      <div class="c-sidebar__footer">
        ...
        <!-- Info de usuario -->
        <div class="c-sidebar__user" id="btn-user-menu">
          <div class="c-sidebar__avatar">${initial(user.username)}</div>
          <div class="c-sidebar__user-info">
            <div class="c-sidebar__user-name">${escHTML(user.username || 'Usuario')}</div>
  ...
  $$('.c-sidebar__item-del').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      EventBus.emit('conv:delete-request', id);
    });
  });
